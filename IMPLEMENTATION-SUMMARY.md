...
- ✅ **Temporal Commerce** (`POST /api/trade`): Merchant NPCs carry deterministic inventories (4–8 `primaryExport` resource slots + 1–2 artifacts) and `personalWealth` (500–3000g) seeded at NPC generation. Buying > 80% of a Merchant's `primaryExport` stock triggers Market Depletion (`shortage` delta). Selling a Relic (age > 300yr) to a Merchant whose wealth exceeds 15 000g triggers Merchant Ascendancy (`plutocracy_candidate` delta → Plutocracy takeover on next `advance_time`). Merchant buy/sell prices multiply by `town.mythos.fearModifier` (live — populated by item 14).
- ✅ **Folklore & Mythos System** (item 14): Towns track `mythos.temporalExposure` incremented by player actions (steal +3, assassinate +20, quest/trade +5). Passing threshold 50 triggers `shadow` (majority violent) or `savior` (majority benevolent) legend generation in `simulateHistory()`. Shadow sets `fearModifier: 2.0` doubling Merchant prices. Savior spawns a `savior_cult` Faction (or links to an existing `ancestral_reverence` Mystery Cult from item 13). Exposure decays 25 points per 50-year unvisited period via `erosion_check()`; dropping below 20 disbands the cult. Active Savior cults accumulate a tithe of 5% `regionalWealth` per decade since last visit.
...
- `GET /api/chunk/:x/:y/chronicle/summary` - Rule-based narrative analysis: `?focus=ruler` (political history, ruler tenures, turbulent years) or `?focus=person&name=NAME` (NPC biography + recursive lineage tree up to 4 generations)
