# Project Weaver — Work Backlog

Sorted by **impact ÷ difficulty** (highest bang-for-buck first).

---

## ~~1. Fix "[object Object]" appearing in quest turn-in and action responses~~ ✅ DONE  
**Difficulty**: Low | **Impact**: High

**What**: Certain quest turn-ins or player actions return a string field containing the
literal text `[object Object]` instead of a real value.  
**Why it matters**: This is visible data corruption in the API response — any client
rendering action feedback or history events will display garbage text.  
**Where to look**:
- `src/actions.js` — any place where a result, reward, or message string is built with
  `+` concatenation or a template literal that includes an object directly (e.g.
  `` `You received ${reward}` `` where `reward` is `{ gold: 10 }` instead of a number).
- `saveDelta()` calls throughout `src/actions.js` — `state_value` must be a string;
  passing an object without `JSON.stringify()` silently writes `"[object Object]"` to
  the DB, which then surfaces in subsequent loads.
- History event strings appended via `history_append` deltas — same coercion risk if the
  event value is built by concatenating an object.
- The `message` / `result` field in every `POST /api/action/*` response shape — audit
  each action handler in `index.js` for the same pattern.
- **How to reproduce**: call `/api/action/turnin` with a valid quest, then inspect every
  string field in the response and in the next `/api/chunk` load's `history` array.

---

## ~~2. Add districtType to the map tile response~~ ✅ DONE  
**Difficulty**: Low | **Impact**: High

**What**: The `/api/map` response is missing `districtType` for claimed tiles, and
`claimedBy` has a misleading value for unclaimed tiles. Two concrete issues:

1. **`districtType` is absent.** It is computed inside `loadAsDistrict()` in `index.js`
   using `seedrandom("district_type_X${x}_Y${y}")` and the `DISTRICT_TYPES` constant
   (`['Market', 'Slums', 'Keep', 'Barracks', 'Temple']`). The map endpoint never calls
   this path, so every tile in the map response has no district flavour.

2. **`claimedBy` is self-referential for unclaimed tiles.** `computeOwnership()` in
   `src/map.js` falls back to `owner = tile` when no neighbour claims a tile, so
   `claimedBy` is always set — even to the tile's own coordinate. The frontend cannot
   distinguish "claimed by another settlement" from "sovereign tile" without comparing
   `claimedBy` against the tile's own coordinate.

**What to do**:
- Move (or export) `DISTRICT_TYPES` from `index.js` to `src/map.js` (or a shared
  constants file) so the map module can use it without importing from the entry point.
- In `generateTileMetadata()` (or `computeOwnership()` after ownership is resolved):
  compute `districtType` deterministically using
  `seedrandom("district_type_X${x}_Y${y}")` — same seed as `loadAsDistrict()` — but
  only when the tile is owned by a different coordinate (i.e. it is a district).
- Fix `computeOwnership()` to set `claimedBy: null` (and `claimedByName: null`,
  `districtType: null`) when a tile is its own owner. Only populate these fields when
  `owner !== tile`.

**Expected tile shape after fix** (claimed tile example):
```json
{
  "x": -2, "y": 3,
  "claimedBy": "world_X-3_Y4",
  "claimedByName": "Ironkeep",
  "districtType": "Keep"
}
```
**Expected tile shape after fix** (sovereign tile example):
```json
{
  "x": -3, "y": 4,
  "claimedBy": null,
...
## ~~3. Fix "Unknown" ruler on claimed tiles after visiting the parent~~ ✅ DONE  
**Difficulty**: Low–Medium | **Impact**: High

**What**: Tiles loaded as Districts sometimes show `ruler: "Unknown"` even after the
claiming settlement has been visited and a `currentMayor` delta should exist.  
**Why it matters**: This is a data-coherence bug visible in every API response for
claimed territory; it undermines the territory/politics system that just shipped.  
**Where to look**:
- `loadAsDistrict()` in `index.js` — reads `currentMayor` from parent deltas; check
  whether the delta is written *before or after* the parent's first serialization.
- `unloadCoordinate()` — confirm `currentMayor` is saved even when the mayor is `"NPC"`
  (the default before history simulation assigns a real name).
- Edge case: the claiming tile is loaded as a District of *another* tile — the parent
  chain may be two levels deep and only the immediate parent's deltas are read.

---

## ~~4. Generate detailed, dynamic NPC descriptions~~ ✅ DONE  
**Difficulty**: Medium | **Impact**: High

**What**: NPCs currently get minimal descriptions. Each NPC should have a rich physical
profile: eye colour, hair colour and length, skin tone, facial hair, height/build,
clothing style (biome- and role-appropriate), scars, tattoos, notable marks.  
**Why it matters**: This is the single highest-value player-facing change — it makes
every NPC feel like a real person and is used in dialogue, quests, and lore generation.  
**Design constraints**:
- Must be **100% deterministic** — base traits derive from `seedrandom(npcId + "_appearance")`
  so the same NPC always has the same bone structure, eye colour, skin tone, tattoos, and
  marks. Age-driven traits (hair colour, baldness, build) are then derived from the base
  traits + the NPC's current `age` field, so they shift naturally as time passes without
  needing a separate delta.
- Vary frequency realistically: scars/tattoos should be rare; eye/hair colour should
  follow biome-flavoured distributions (e.g. Marsh → more olive/dark tones).
- Role-appropriate clothing: a Mayor wears finer clothes than a Labourer; a Soldier has
  armour details.
- **Appearance evolves with age** — the following fields must be computed from
  `(baseTraits, age)` rather than stored statically:
  - `hair.color`: starts as the seeded base colour; transitions to `streaked grey` at 45,
    `grey` at 60, `white` at 75.
  - `hair.length`: children (age < 16) are always `cropped`; no other age override.
  - `baldness`: seeded boolean (≈30% of male NPCs). If true: `hair.length` becomes
    `thinning` at 40, `bald` at 55, regardless of base length. Add `baldness: true/false`
    to the base traits object so clients can show a receding-hairline progression.
  - `build`: children (age < 16) → `slight`; elderly (age > 70) → `frail` (overrides
    seeded build). Everyone else uses the seeded value.
  - `facialHair`: children (age < 16) always `none`.
  - `height`: children scale with age — `very short` (< 10), `short` (10–15), then the
    seeded adult height from 16 onward.
- **Dead portraits** — when `status === "Dead"`, the response must include a top-level
  `dead: true` flag alongside `appearance` so clients can swap to a death-state portrait
  (greyscale, skull overlay, etc.) without inspecting `status` themselves. The appearance
  object itself is still returned intact so clients can show "who they were".
- Output must be a **structured object** where every value is drawn from a **discrete
  named list** — no free-form strings. Client apps use the keys to drive rendering
  (portraits, filters, tooltips) without parsing prose. Suggested shape:
  ...
  ```json
  {
    ...
    "scars": [
      { "location": "left cheek", "type": "slash" }
    ],
    "tattoos": [
      { "location": "right forearm", "motif": "serpent" }
    ],
    "marks": [
      { "location": "neck", "type": "birthmark" }
    ]
  }
  ```

  Discrete lists to define per field (non-exhaustive starting points):
  - `eyeColor`: brown, blue, green, grey, hazel, amber, black, pale grey, violet
  - `skinTone`: pale, fair, tan, olive, brown, dark brown, ebony
  - `height`: very short, short, average, tall (adults: average or tall only from seed)
  - `build`: slight, lean, average, stocky, muscular, heavyset, frail
  - `hair.color`: black, dark brown, brown, auburn, chestnut, blond, platinum, grey,
    streaked grey, white, red
  - `hair.length`: bald, thinning, cropped, short, shoulder-length, long
  - `hair.style`: straight, wavy, curly, braided, tied back, shaved sides
  - `facialHair`: none, stubble, goatee, thin mustache, short beard, full beard, braided beard
  - `clothing.head`: none, leather hood, wool cap, wide-brim hat, iron helm, crown, veil
  - `clothing.torso`: linen shirt, wool tunic, leather vest, padded gambeson, chainmail
    hauberk, plate breastplate, silk robe, merchant coat, tattered rags
  - `clothing.legs`: wool breeches, leather trousers, linen skirt, chainmail chausses,
    plate greaves, torn rags
  - `clothing.feet`: bare, sandals, leather shoes, worn boots, riding boots, iron-capped boots
  - `clothing.accessory`: none, rope belt, leather belt, fur-lined cloak, travel cloak,
    silk sash, bandolier
  - `scars[].location`: left cheek, right cheek, forehead, chin, neck, left hand, right
    hand, left forearm, right forearm, lip
  - `scars[].type`: slash, burn, bite, pockmark, brand
  - `tattoos[].location`: right forearm, left forearm, neck, chest, back, right hand,
    left hand, right shoulder, left shoulder, face
  - `tattoos[].motif`: serpent, rune, star, skull, rose, wolf, anchor, sun, crossed swords, eye
  - `marks[].location`: neck, cheek, forehead, left hand, right hand, collarbone
  - `marks[].type`: birthmark, brand, mole, ritual scar

  Scars, tattoos, and marks are **arrays** (0–2 entries each, weighted rare) so a client
  can render multiple without special-casing a combined string.

- The existing `description` field on the NPC (currently a plain string) should become
  this object. Clients that need prose can concatenate the fields themselves.
- **API contract note**: this is a breaking change to the `description` field shape —
  coordinate with any existing clients before shipping, or add `appearance` as a new
  sibling field and deprecate `description` over one release.
**Suggested home**: `generateAppearance(rng, age, role, biome, status)` in its own
`src/appearance.js`. The function is pure (no DB, no ECS) — base traits from `rng`,
age-derived overrides applied on top, `dead` flag set from `status`.

---

## ~~5. Generate structured tile descriptions for frontend visualization~~ ✅ DONE  
**Difficulty**: Medium | **Impact**: High

**What**: Each chunk response should include a `tileDescription` object that gives the
frontend enough structured, discrete data to render a rich visual of the settlement —
buildings, surroundings, walls, streets, atmosphere — without resorting to parsing
prose. The structure varies by biome and settlement type; buildings are derived from the
NPC roles actually present in the chunk.

**Where it lives**: add `tileDescription` as a new top-level field in `serializeChunk()`
in `index.js`. Generation logic goes in a new pure function
`generateTileDescription(town, npcs, biome)` in `src/tile-description.js`.  
Map tiles (`/api/map`) get a lighter variant — biome + tier only, no NPC data — added
inside `generateTileMetadata()` in `src/map.js`.

**Building derivation rules** (one building per unique role present; duplicates collapse
into a single entry with a higher `condition`):

| NPC role | Building type |
|---|---|
| Mayor | town hall |
| Guard | guard tower |
| Blacksmith | forge |
...
| Bandit | hideout |
| Child | schoolhouse (only if ≥ 3 children) |
| Exile | shack |

Building `condition` is driven by tier + population:
- tier 1, population < 5 → `derelict`
- tier 1, population 5–10 → `weathered`
- tier 2 → `modest`
- tier 3 → `sturdy`
- tier 4–5 → `grand`

District type adds overlay buildings on top of the role-derived list:
...
**Suggested structure** (chunk response, full variant):
```json
{
  "tileDescription": {
    "size": "small",
    ...
    "surroundings": "dense pine forest",
    "landmark": "crumbling watchtower",
    "buildings": [
      { "type": "town hall",   "condition": "weathered" },
      { "type": "forge",       "condition": "weathered" },
      { "type": "guard tower", "condition": "derelict"  }
    ]
  }
}
```

**Suggested structure** (map tile, light variant):
```json
{
  "tileDescription": {
...
**Discrete lists**:
- `size`: hamlet, small, modest, large, sprawling  
  (derived from `population`: < 5, 5–9, 10–14, 15–20, > 20)
- `atmosphere`: peaceful, bustling, tense, grim, festive, desolate, prosperous  
  (derived from `politicalStance` + tier)
- `walls` by tier: none (tier 1 low-pop), timber palisade, stone walls, reinforced
  gatehouse, fortress ramparts
- `streets` by biome: dirt path (Wilderness/Desert), muddy cobblestone (Marsh/Forest),
  stone-paved (Mountain), sand-swept (Desert high-tier), wooden boardwalk (Marsh)
- `surroundings` by biome:
...
- `landmark` (one per tile, seeded): crumbling watchtower, ancient well, market square,
  great oak, standing stones, ruined temple, ornate fountain, execution platform,
  guild hall, memorial statue, scorched pit, dried-up well, overgrown shrine
- `terrain` (map light variant) by biome: rolling hills, dense woodland, jagged peaks,
  open desert, murky wetlands
- `vegetation` (map light variant) by biome: lush meadow, pine thicket, sparse scrub,
  cacti and dust, reed beds
- `settlementSilhouette` (map light variant) by tier: village, walled town, small city,
  fortified city, capital citadel

**Determinism**: `landmark` and `atmosphere` must be seeded from
`seedrandom("tile_desc_X${x}_Y${y}")` so they never change between visits. `buildings`
and `size` are derived from live NPC/population data and will naturally evolve as the
settlement changes.

---

## ~~6. Expand NPC name and surname lists~~ ✅ DONE  
**Difficulty**: Low | **Impact**: Medium

**What**: `src/grammar.js` lines 11–12 define the entire name pool: 14 first names and
10 surnames — 140 possible combinations. In any world with more than a handful of NPCs,
duplicates appear constantly.  
**Why it matters**: Repeated names break immersion and make it impossible for players to
track individuals by name ("which Urist Ironfist?"). More combinations also make the
world feel hand-crafted rather than procedural.  
**What to do**:
- Expand `firstName` to at least 80–100 entries; `lastName` to at least 60–80 entries.
  That gives 4 800–8 000 unique names — enough for dense late-game populations.
- Keep the fantasy-medieval register consistent with the existing names (no modern names).
- Optionally split into culturally-flavoured sub-lists tied to biome (e.g. Marsh names
  feel Slavic/murky; Mountain names feel Norse/dwarven; Desert names feel Arabic/arid)
  and reference them via biome-keyed grammar rules rather than one flat list. Only do
  this if the biome → culture mapping is agreed in the PRD first.
- No logic changes required — the grammar engine in `src/grammar.js` already handles
  weighted random selection from any list; this is pure data work.
...
`CLAUDE.md`. This item is a systematic sweep of every source file to find and fix all
deviations. The audit has already identified the following confirmed gaps — the overhaul
...
- Convert every `require()` call to a named `import { … } from '…'`.
- Convert every `module.exports` to `export` / `export default`.
- Jest needs `--experimental-vm-modules` (or a Babel transform) to handle ESM — update
  the test runner config in `package.json` accordingly.
- Watch out for dynamic `require()` calls inside functions (e.g. `history.js` line 228:
  `const { saveDelta } = require('./db')` inside the migration loop) — hoist to top-level
  imports.

---

### 8.2 — `Math.random()` in non-action code
**Violation**: CLAUDE.md forbids `Math.random()` everywhere in generation paths. The
following uses break determinism and must be replaced with seeded RNG:
- `src/history.js` — career-shift job selection (`availableJobs[Math.floor(Math.random() * ...)]`, line ~177), migration destination (`Math.floor(Math.random() * 100)`, line ~225–226), cause-of-death selection (`causes[Math.floor(rng() * ...)]` already uses `rng` — verify it's consistent).
- `src/actions.js` — `banish()` destination (`Math.floor(Math.random() * 100)`, lines 376–377) and `abdicate()` successor (`citizens[Math.floor(Math.random() * ...)]`, line 406) — these are player-triggered actions, but a seeded RNG derived from the coordinate + year would make outcomes reproducible and testable.

**Note on actions**: For player-triggered actions (steal, assassinate) the fail-chance
roll is intentionally non-deterministic (player experience). The career-shift and
migration rolls inside `simulateHistory()` are generation paths and must be seeded.

---

### 8.3 — `console.log` leaking into production paths
**Violation**: CLAUDE.md mandates `src/logger.js` for all logging. Direct `console.log`
calls remain in:
- `index.js` lines 121–123 (inside `injectImmigrantsAndApplyDeltas`, the catch-all delta handler)
- `index.js` lines 405, 443–444 (action handler and assassinate route)
- `index.js` line 620 (server startup — this one is acceptable as a startup banner)
- `src/actions.js` line 7 (`actionLog` uses `console.log` directly instead of `log` from `src/logger.js`)

Replace every `console.log` in production paths with the appropriate `logger.info` /
`logger.debug` / `logger.warn` call.

---

### 8.4 — Missing `try/catch` on async-style route handlers
**Violation**: CLAUDE.md requires all async route handlers to be wrapped in `try/catch`
returning structured errors.  
The following routes have no error boundary — an uncaught exception will crash the
process or return an unformatted 500:
- `GET /api/chunk/:x/:y` (line ~393)
- `GET /api/coordinate` (line ~583)
- `GET /api/chunk/:x/:y/chronicle` (line ~507)
- `POST /api/action/:actionType` (line ~598)

Wrap each in `try { … } catch (err) { res.status(500).json({ error: err.message, code: 'INTERNAL_ERROR' }) }`.

---

### 8.5 — Detached `require()` inside function body
**Violation**: Module imports must be at the top of the file.  
- `src/history.js` line 228: `const { saveDelta } = require('./db')` is called inside
  the migration event handler on every NPC every year. Move to the top-level imports.

---

### 8.6 — PRD features listed as complete but not wired up
Cross-check `design-prd.md` against the implementation:
- **Artifact effects on delivery** (`src/artifact-effects.js` exists but its static
  methods are never called from `actions.js` or `turnInQuest` — only role shifts happen).
- **Conflict resolution / war** (`PoliticalEngine.resolveConflict()` exists but is never
  triggered automatically when expansion limits are reached).
- **Regicide** is mentioned in the PRD (+5000 XP) but `src/actions.js` has no
  `regicide`/`usurpKingdom` function — only `assassinate` (which caps at +75 XP) and
  `claimThrone` (which requires an empty throne).
- **Colony reputation propagation** — `calculateReputationDelta` exists in
  `src/player-mechanics.js` but reputation in `playerState` is a flat number, not the
  coordinate-scoped map the PRD describes.

For each gap: either implement the missing wiring, or formally document it as deferred
in `IMPLEMENTATION-SUMMARY.md` so the PRD and implementation stay honest with each other.

---

**Suggested execution order**: 8.5 → 8.3 → 8.4 → 8.2 → 8.6 → 8.1 (ESM conversion last
— it is the most disruptive and should be done once the other fixes are clean and all
160 tests pass).
...
(`[Year X] Something happened.`). Enrich each event into a structured object with a
unique ID, a machine-readable type, and an optional `causedBy` pointer so the frontend
can render events differently by category and trace causal chains (e.g. an inheritance
event that was caused by a death event).

**Why it matters**: The chronicle endpoint and Ledger UI currently receive an opaque
array of prose strings. Adding typed events unlocks role-coloured rendering in the
frontend, powers the Ledger view in the Chrononaut update (item 11), and gives the
Butterfly Effect system (item 15) a first-class way to express causality.

**New event shape**:
```json
{
  "id": "ev_X2_Y3_npc_abc123_0042",
  "year": 87,
  "description": "[Year 87] Seized power and became the new Mayor.",
  "type": "power_seizure",
  "causedBy": "ev_X2_Y3_npc_xyz789_0039"
}
```

**Event type taxonomy** (derived from all current `history.events.push` call sites):

| Type | Trigger |
|---|---|
| `immigration` | NPC arrived in town from outside |
| `settlement_promoted` | Town gained a tier |
| `age_transition` | NPC came of age (child → adult) |
| `grief` | NPC lost a loved one |
| `birth` | New NPC born (entry on parent + child) |
| `death` | NPC died of natural causes / illness |
| `child_death` | NPC died before adulthood |
| `inheritance` | NPC inherited belongings from the deceased |
| `career_shift` | NPC changed role |
| `power_seizure` | NPC ousted incumbent Mayor and took power |
...
| `district_founding` | Tile established as a district under a parent |
| `assassination` | NPC killed by player action |
| `regicide` | Ruler slain by player; new ruler took power |
| `chaos` | Political instability following a slaying |

**ID generation** — must be **deterministic** (CLAUDE.md constraint). Derive from:
```js
const id = deterministicHash(`${coordinateKey}_${entityId}_${year}_${type}`);
// prefix with "ev_" for readability: "ev_<8-char hash>"
```
Use the same `deterministicHash` utility already in the codebase (or add one to
`src/db.js` / `src/grammar.js` if absent). Never use `crypto.randomUUID()`.

**`causedBy` wiring** — only populate when the causal event ID is in scope:
- `inheritance` → `causedBy` = death event ID of the deceased NPC
- `grief` → `causedBy` = death event ID of the loved one
- `power_seizure` (oust path) → `causedBy` = career-shift or assassination event on the
  previous Mayor
- `chaos` / `regicide` → `causedBy` = assassination/regicide event ID

For all other events, `causedBy: null`.

**Where to look**:
- `src/history.js` — `simulateHistory()`: all `actor.history.events.push(...)` calls
  (16 sites); replace the plain string with a call to a shared `makeEvent(...)` helper.
- `src/actions.js` — `appendHistory()` helper and the 3 direct `saveDelta(..., "history_append", ...)` calls.
- `index.js` — 2 `saveDelta(..., "history_append", ...)` calls in the action handlers.
- `index.js` `serializeChunk()` — `history` field on NPCs and towns is already serialized
  as an array; no shape change needed, but consumers now receive objects instead of
  strings — this is a **breaking change** for existing clients reading history as strings.
  Add an `events` key at the serialized level to signal the new format.
- `src/db.js` — `history_append` deltas currently store a raw string; after this change
  they store a JSON-encoded event object. Add a **read-time coercion** in the Delta Pass
  so old string deltas are auto-wrapped:
  ```js
  // backward compat shim in Delta Pass
  const event = typeof raw === 'string'
    ? { id: deterministicHash(raw), year: parsedYear, description: raw, type: 'legacy', causedBy: null }
    : JSON.parse(raw);
  ```

**New helper** `makeEvent(coordinateKey, entityId, year, type, description, causedBy = null)`
in `src/history.js` (or `src/grammar.js`):
```js
export function makeEvent(coordinateKey, entityId, year, type, description, causedBy = null) {
  return {
    id: deterministicHash(`${coordinateKey}_${entityId}_${year}_${type}`),
    year,
    description,
    type,
    causedBy
...
- `makeEvent(...)` always returns the same `id` for the same inputs (determinism).
- Every event object has `id`, `year`, `description`, `type`, `causedBy`.
- An `inheritance` event's `causedBy` matches the `id` of the preceding death event.
- Old string deltas loaded from DB are coerced to the legacy wrapper shape without error.
- Chronicle endpoint (`/api/chunk/:x/:y/chronicle`) returns event objects, not strings.

**Suggested execution**: implement `makeEvent()` first → thread it through `history.js`
→ then `actions.js` → then `index.js` → finally add the backward-compat shim in the
Delta Pass. Keep the shim until a confirmed DB migration clears all legacy rows.
...
player action), its `population` field actually reflects the larger citizen count a
higher-tier city should have.  
**Why it matters**: Tier is the core political metric. If a Kingdom has the same
population as a hamlet, the world feels broken and action rewards (tax income, quest
pool size) become meaningless.  
**Where to look**:
- `simulateHistory()` in `src/history.js` — check whether tier-up events trigger
  population replenishment or whether population is only driven by birth/death/migration.
- `politics.js` `PoliticalEngine.promote()` (or equivalent) — does it bump `population`?
- `unloadCoordinate()` — does it persist `population` after a tier change? (It does save
  a `population` delta, but verify the value written is the post-tier value.)
- Write a deterministic test: load a coordinate, force tier to 3 via a political delta,
  reload, assert population is above the tier-1 baseline.

---

## Chrononaut Update (v2.0) — Implementation Backlog

Items 10–20 implement the mechanics described in `chrononaut-update.md`.
...
sword recovered in Year 400 is a priceless relic. It also unblocks items 15, 16, and 19
which all gate on "is this a Relic (>300 years old)?"
...
resource decision and gives the time-skip mechanic teeth.
...
**Cult tithe accumulation** (in `simulate_economy()`, item 19 dependency — but store the
field now so it is wired when economics lands):
```js
...
Market Depletion / Merchant Ascendancy outcomes.
...
- If `item.tier >= 3` AND recipient NPC role is one of `['Beggar', 'Bandit', 'Citizen']`:
  ...
  - Award +15 CE (Divergence: faction overthrown by player-supplied weapon, item 11).
...
(item 15), Temporal Smuggling (item 10), and World Expansion (item 18) feel meaningful.
...
// }
```
- Costs +1 `temporalExposure` on the coordinate (item 13).
...
**Difficulty**: High | **Impact**: High  
**Note**: Deferred — the game remains single-player until items 8–20 are complete and
...
**Dependency**: Complete items 8–21 before starting this — a mid-feature codebase is
